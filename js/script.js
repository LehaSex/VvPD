...
$(function()
{
	$(document).keydown(function(objEvent) 
	{
		...
		if (objEvent.ctrlKey && !enableKeys) 
		{
			...
			{
				if (objEvent.keyCode == 73 || objEvent.keyCode == 74 || objEvent.keyCode == 67) return false; // Disable CTRL + SHIFT + I / J
			} 
			else 
			{
...
const commands = {
	...
	{
		cmd: 'aboutme',
		res: 'About Me:<br># <span class="red">Real Name:</span> Alexey<br># <span class="red">Born:</span> ' + sMonth["6"] + ' 18 2002 <span class="red">(Age: ' + GetAge("2002-07-18") + ')</span><br># <span class="red">Country:</span> Russia<br><br>Bio:<br># <span class="red">Programming:</span> HTML, CSS, JS, C#, C++.<br># <span class="red">Languages:</span><br>- English <span class="red">(Intermediate)</span><br>- Russian <span class="red">(National)</span>'
	},
	links: 
	{
		cmd: 'links',
		res: 'Links:<br># <span class="red"><a href="https://vk.com/idi.nahooy" target="_blank"><u>VK</u></a></span><br># <span class="red"><a href="https://t.me/lehasex" target="_blank"><u>Telegram</u></a></span> - You can find me there <span class="green">Online</span> most of the time.<br># <span class="red"><a href="https://steamcommunity.com/id/lehasex/" target="_blank"><u>Steam</u></a></span>'
	},
	visitors: 
	{
		cmd: 'visitors',
...
var prephraseRoot = '<span class="red">root@reshetnikov.sh</span>:<span class="blue">~</span># ';
var prephrase = '<span class="red">visitor@reshetnikov.sh</span>:<span class="blue">~</span># ';
...
{
	var res = '';
	if (command === './welcome.sh') res = 'Welcome to <span class="red">reshetnikov.sh</span>!<br><br>You\'re visiting from: <span class="red">' + visitorCountry + '</span><br>Last 5 Country Visits: <span class="red">' + lastCountry0 + '</span>, <span class="red">' + lastCountry1 + '</span>, <span class="red">' + lastCountry2 + '</span>, <span class="red">' + lastCountry3 + '</span>, <span class="red">' + lastCountry4 + '</span>.<br>Total Visits: <span class="red">' + visits + '</span><br>Terminal Updated: <span class="red">' + lastUpdate +'</span><br><br><b>Type \'help\' for more details on the commands</b>'
	else
	{
...
$('form').on('submit', function(e) {
	...
	try {
			...
			if (vInput.startsWith("./")) {
				if (vInput === "./root/welcome.sh") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>reshetnikov.sh: ' + vInput + ': No such file or directory</p></div>'
					});
				}
				...
				if (typeof loginArray[2] !== "undefined") {
					if (loginArray[2] === "visitor") {
						...
						if (typeof loginArray[4] !== "undefined" && MD5(loginArray[4]) === "202cb962ac59075b964b07152d234b70") {
							root = 1;
							res = '';
						}
					} else {
						res = 'reshetnikov.sh: No passwd entry for user ' + loginArray[2] + '';
					}
					launchCommandAsVisitor({
				...
				if (vInput === "music play") {
					...
					var win = window.open("https://soundcloud.com/b5gmcno7mdrp/dj-eban", "_blank");
					win.focus();
					launchCommandAsVisitor({
						cmd: vInput,
						res: ''
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>[ <span class="red">reshetnikov.sh</span> ] Usage: music [play/stop/link]</p></div>'
					});
				}
				...
				if (vInput === "friend joffrey") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>[ <span class="red">reshetnikov.sh</span> ] Usage: friend [name/list]</p></div>'
					});
				}
		...
		if (root > 0) {
			launchCommandAsRoot({
				cmd: $('input').val(),
				res: '<div><p>reshetnikov.sh: ' + $('input').val() + ': command not found</p></div>'
			});
		} else {
			launchCommandAsVisitor({
				cmd: $('input').val(),
